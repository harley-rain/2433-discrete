...
// recieves: none
// returns: none
// purpose: opens, formats, & names the outfile
...
int main()
{
  ...
  // For different sizes of arrays in increasing order of magnitude
  for (int i = 0; i < MAXSIZES; i++)
    {
      //n is size of array to search and sort
      long n = sizes[i];

      //dynamically allocate array of values
      int* x = new int[n];
      int* y = new int[n];
      genValues(x, n);

      // copy elements of first array to second array
      for (long j = 0; j < n; j++) 
        {
          y[j] = x[j]; 
        }    
...
// Output will show: 
// Binary search scales very efficiently even for large inputs.
// Merge sort becomes significantly more expensive as input size increases.
// Iterative and recursive approaches have similar performance, but recursion introduces slight overhead.
...
// purpose: opens output file and prints page heading
void openFile(ofstream &outFile)
{
    string filename;
